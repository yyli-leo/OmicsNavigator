"""
Re-export stub for pickle compatibility.

Pickle files reference src.data.data_models.
This file defines the same classes for pickle deserialization.
"""
...
class RegionShardData:
    ...
    # Store ONLY integer indices pointing to `self.rois`
    cluster_center_indices: Dict[Tuple[int, int], List[int]] = field(default_factory=dict)

    def __post_init__(self):
        """Runtime validation for data alignment."""
...
class SpatialPivotRegistry:
    """
    The top-level root data structure for ROI registry.
    """
